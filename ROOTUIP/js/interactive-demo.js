...
// Handles all demo interactions, animations, and tour logic
...
function customizeForRole(role) {
    // Adjust visible elements based on role
    if (role === 'executive') {
        // Show high-level metrics, hide technical details
        document.querySelectorAll('.technical-detail').forEach(el => el.style.display = 'none');
    } else if (role === 'operations') {
        // Focus on workflow elements
        switchView('prevention');
    } else if (role === 'it') {
        // Show technical integration details
...
function handleTourAction() {
    // Auto-advance tour after action
...
function startDocumentProcessing() {
    ...
    // Automated process animation (fast)
    setTimeout(() => {
...
function createNewProcessingDoc() {
    // Create new document in stream (simplified for demo)
    ...
    // Continue processing simulation
